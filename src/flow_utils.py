...
class RAFT(nn.Module):
    def __init__(self, model_path='checkpoints/raft_chkpt_00140.pth', num_iters=5, dropout=0):
        super(RAFT, self).__init__()
        
        from flow_models.raft import raft

        # Get location of checkpoints
        raft_path = pathlib.Path(model_path)
        
        # Emulate arguments
        args = argparse.Namespace()
        args.model = raft_path
        args.small = False
        ...
        # Load checkpoint and remove DataParallel prefixes if needed
        checkpoint = torch.load(raft_path, map_location='cpu')
...
def make_flow_model(flow_model='raft', num_iters=5, dropout=0):
    if flow_model == 'raft':
        return RAFT(num_iters=num_iters, dropout=dropout)
    elif flow_model == 'pwc':
        return PWC()
    elif flow_model == 'gmflow':
        return GMFlow()
    elif flow_model == 'arflow':
        return ARFlow()
    else:
