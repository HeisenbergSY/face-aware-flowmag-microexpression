...
from src.models.model_zoo import UNet
from src.losses import MMLoss
...
class MotionMagModel(nn.Module):
    def __init__(self, config):
        ...
        if config.train.is_training:
            self.training = True
            self.mm_loss_fn = MMLoss(config, flow_model=config.train.flow_model)
    ...
    def forward(self, frames, alpha=None, mask=None):
        ...
        # Compute Losses
        if self.training:
            loss_mag, loss_color, info = self.mm_loss_fn(preds, frames, alphas, mask=mask)
            loss_mag = loss_mag.mean()
            loss_color = loss_color.mean()
            loss = self.config.train.weight_mag * loss_mag + \
                    self.config.train.weight_color * loss_color
            # Add stuff to info
    ...
    def freeze_early_layers(self):
        for name, param in self.generator.named_parameters():
            if 'down' in name or 'middle' in name:
                param.requires_grad = False
            else:
                param.requires_grad = True  # Ensure decoder remains trainable

        print("✔️ Encoder and middle layers frozen. Decoder remains trainable.")
...
class AlphaAppender(nn.Module):
    ...
    def forward(self, frames, alpha=None, mask=None):
        ...
        if mask is not None:
            ...
            # Expand to (B, pos_dim, H, W)
            mask = mask.expand(-1, self.pos_dim, -1, -1)

            # Get "zero" motion mag
            zero_alpha = self.get_positional_encoding(torch.ones(b).to(frames.device))
            zero_alpha = repeat(zero_alpha, 'b d -> b d h w', h=h, w=w)

            # Replace masked regions with zero_alpha
