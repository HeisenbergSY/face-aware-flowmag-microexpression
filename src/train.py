...
from src.model import MotionMagModel
from src.dataset import TrainingFramesDataset, RepeatDataset, get_dataloader
from src.myutils import AverageMeter, log_images
...
def main():
    ...
    # Logging
    config.loss_names = ['Loss', 'MagLoss', 'ColorLoss', 'LandmarkLoss']
    ...
    trainset = TrainingFramesDataset(config.log.inference_dir, im_size=config.data.im_size)
    trainset = RepeatDataset(trainset, config.data.repeat_factor)
    valset = TrainingFramesDataset(config.log.inference_dir, im_size=config.data.im_size)
    ...
    valloader = DataLoader(valset, batch_size=config.data.batch_size, shuffle=False, num_workers=0, drop_last=True)
    ...
    def train(model, trainloader, optimizer, config, writer, epoch_num):
        ...
        for cur_iter, data in enumerate(tqdm(trainloader)):
            ...
            # Model forward
            output = model(frames, mask=mask)
            if isinstance(output, tuple) and len(output) == 3:
                preds, loss, info = output
            else:
                preds = output
                loss = torch.tensor(0.0)  # Dummy loss if not returned
                info = {}
            ...
            if 'loss_color' in info:
                metrics['ColorLoss'].update(info['loss_color'].mean().item())
            if 'loss_landmark' in info:
                metrics['LandmarkLoss'].update(info['loss_landmark'].mean().item())
        ...
        return {
            'final_frames': frames,
            'final_preds': preds,
            'last_iter': global_iter  # for logging images later
        }
    ...
    def validate(model, valloader, config):
        ...
        with torch.no_grad():
            for cur_iter, data in enumerate(tqdm(valloader)):
                ...
                # Flexible unpacking depending on training mode
                if isinstance(output, tuple) and len(output) == 3:
                    preds, loss, info = output
                elif isinstance(output, tuple) and len(output) == 2:
                    loss, info = output
                    preds = info.get('final_preds', None)
                else:
                    # Only predictions (no loss returned)
                    preds = output
                    loss = torch.tensor(0.0)
                    info = {}

                loss = loss.mean() if isinstance(loss, torch.Tensor) else torch.tensor(0.0)

                # Logging available losses
                metrics['Loss'].update(loss.item())
                if 'loss_mag' in info:
                    metrics['MagLoss'].update(info['loss_mag'].mean().item())
                if 'loss_color' in info:
